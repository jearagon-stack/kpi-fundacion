...
# ==========================================
# CONSTANTES CONTABLES CID CAMPUS
# ==========================================
...
def gen_excels_memory(partidas_dict, mes_proceso):
    ...
    for categoria, tabs_dict in partidas_dict.items():
        ...
        buffers[f"Partidas_CID_{nombres_amigables.get(categoria, categoria)}_Mes_{mes_proceso}.xlsx"] = output.getvalue()
    return buffers

# ==========================================
# INTERFAZ Y PROCESAMIENTO
# ==========================================
def mostrar_modulo_cid_campus():
    st.title("🏫 Módulo de Inventarios - CID CAMPUS")
    ...
    with tab_carga:
        ...
        with col_m1: mes_proceso = st.selectbox("Mes:", range(1, 13), index=date.today().month - 1, key="m_cid")
        with col_m2: anio_proceso = st.number_input("Año:", min_value=2024, value=date.today().year, key="y_cid")

        st.markdown("---")
        st.subheader("Archivos Base (Generación Nexus y Auditoría)")
        
        arch_mov_inv = st.file_uploader("1. Reporte de Movimientos (Inventario)", type=["xls", "xlsx"], key="file_inv_cid")
        
        col_a1, col_a2 = st.columns(2)
        with col_a1:
            arch_kardex = st.file_uploader("2. Kardex Consolidado (Auditoría)", type=["xls", "xlsx"], key="file_kar_cid")
        with col_a2:
            arch_cat = st.file_uploader("3. Maestro Categorías (Opcional)", type=["xls", "xlsx"], key="file_cat_cid")

        btn_scan = st.button("🔍 Procesar y Generar Partidas", type="primary", use_container_width=True, key="btn_cid")
            ...
            with st.spinner("Analizando información y procesando datos..."):
                ...
                try:
                    ...
                    if arch_mov_inv:
                        ...
                        if not df.empty:
                            ...
                            for _, row in df.iterrows():
                                ...
                                is_sender_cid = raw_sender in UNIDADES_CID_CAMPUS_SET
                                is_receiver_cid = raw_receiver in UNIDADES_CID_CAMPUS_SET
                                
                                if not (is_sender_cid or is_receiver_cid) or (is_sender_cid and is_receiver_cid) or 'SERVICIO' in category: continue
                                ...
                                if es_ajuste_o_consumo:
                                    if is_receiver_cid and not is_sender_cid:
                                        desc = f"RECONOCIMIENTO DE ENTRADA POR AJUSTE DE PRODUCTO DE CID CAMPUS, MES {mes_proceso} DE {anio_proceso}."
                                    ...
                                    elif is_sender_cid and not is_receiver_cid:
                                        desc = f"RECONOCIMIENTO DE SALIDA POR AJUSTE O CONSUMO DE PRODUCTO DE CID CAMPUS, MES {mes_proceso} DE {anio_proceso}."
                                ...
                                elif is_sender_cid and es_traslado:
                    ...
                    if arch_mov_inv:
                        ...
                        st.session_state['tab_results_cid'] = True
