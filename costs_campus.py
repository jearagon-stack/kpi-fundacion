...
# ==========================================
def mostrar_modulo_campus():
    st.title("🏫 Módulo de Inventarios - CID Campus")
    st.info("Automatización de Partidas Contables de Inventario para CID Campus.")
    ...
    with tab_carga:
        ...
        with col_m1: mes_proceso = st.selectbox("Mes:", range(1, 13), index=date.today().month - 1, key="m_campus")
        with col_m2: anio_proceso = st.number_input("Año:", min_value=2024, value=date.today().year, key="y_campus")

        st.markdown("---")
        arch_mov_inv = st.file_uploader("Subir Reporte de Movimientos (Inventario)", type=["xls", "xlsx"], key="file_inv_campus")
        btn_scan = st.button("🔍 Procesar Auditoría", type="primary", use_container_width=True, key="btn_campus")
            ...
            with st.spinner("Analizando información..."):
                try:
                    ...
                    for _, row in df.iterrows():
                        ...
                        is_sender_unidad = raw_sender in UNIDADES_CAMPUS_SET
                        is_receiver_unidad = raw_receiver in UNIDADES_CAMPUS_SET
                        
                        # Filtros de exclusión (ignora movimientos internos o servicios)
                        ...
                        if 'AJUS' in tipo or not es_traslado:
                            if is_receiver_unidad: 
                                ...
                                desc = f"RECONOCIMIENTO DE SALIDA POR AJUSTE DE PRODUCTO DE CID CAMPUS, MES {mes_proceso} DE {anio_proceso}."
                    ...
                    st.session_state['nexus_campus_buffers'] = gen_excels_memory(partidas_dict, mes_proceso)
                    st.session_state['tab_results_campus'] = True
                except Exception as e: st.error(f"Error técnico: {e}")

    with tab_liquidacion:
        if st.session_state.get('tab_results_campus', False):
            for f, b in st.session_state['nexus_campus_buffers'].items():
                st.download_button(label=f"⬇️ Descargar {f}", data=b, file_name=f, key=f"dl_campus_{f}")