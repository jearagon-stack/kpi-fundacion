...
# (Ajusta estos nombres si en el Excel de Nexus salen diferentes)
...
def mostrar_modulo_terraza():
    ...
    st.info("Automatización de Partidas Contables de Inventario para Operación Estándar.")
    ...
    with tab_carga:
        ...
        with col_m1: mes_proceso = st.selectbox("Mes:", range(1, 13), index=date.today().month - 1, key="m_terraza")
        with col_m2: anio_proceso = st.number_input("Año:", min_value=2024, value=date.today().year, key="y_terraza")

        st.markdown("---")
        arch_mov_inv = st.file_uploader("Subir Reporte de Movimientos (Inventario)", type=["xls", "xlsx"], key="file_inv_terraza")
        btn_scan = st.button("🔍 Procesar Auditoría", type="primary", use_container_width=True, key="btn_terraza")
            ...
            with st.spinner("Analizando información..."):
                try:
                    ...
                    for _, row in df.iterrows():
                        ...
                        is_sender_unidad = raw_sender in UNIDADES_TERRAZA_SET
                        is_receiver_unidad = raw_receiver in UNIDADES_TERRAZA_SET
                        
                        # Filtros de exclusión (ignora movimientos internos de Terraza a Terraza o servicios)
                        ...
                        if 'AJUS' in tipo or not es_traslado:
                            if is_receiver_unidad: 
                                ...
                                desc = f"RECONOCIMIENTO DE SALIDA POR AJUSTE DE PRODUCTO DE TERRAZA, MES {mes_proceso} DE {anio_proceso}."
                    ...
                    st.session_state['tab_results_terraza'] = True
                except Exception as e: st.error(f"Error técnico: {e}")

    with tab_liquidacion:
        if st.session_state.get('tab_results_terraza', False):
            for f, b in st.session_state['nexus_terraza_buffers'].items():
                st.download_button(label=f"⬇️ Descargar {f}", data=b, file_name=f, key=f"dl_terraza_{f}")