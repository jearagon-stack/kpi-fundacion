...
def mostrar_modulo_gerencia():
    ...
    with tab_carga:
        ...
        with col_f1:
            arch_mov_inv = st.file_uploader("1. Reporte de Movimientos (Inventario)", type=["xls", "xlsx"], key="file_inv_ger")
        with col_f2:
            arch_ventas = st.file_uploader("2. Reporte de Ventas (Consignación)", type=["xls", "xlsx"], key="file_ven_ger")

        st.subheader("Archivos de Auditoría (Validación 1% y $0.02)")
        col_a1, col_a2 = st.columns(2)
        with col_a1:
            arch_kardex = st.file_uploader("3. Kardex Consolidado", type=["xls", "xlsx"], key="file_kar_ger")
        with col_a2:
            arch_cat = st.file_uploader("4. Maestro Categorías (Opcional)", type=["xls", "xlsx"], key="file_cat_ger")

        btn_scan = st.button("🔍 Procesar y Generar Partidas", type="primary", use_container_width=True, key="btn_ger")
        ...
        if btn_scan:
            ...
            with st.spinner("Analizando información y cruzando datos de Kardex..."):
                ...
                try:
                    ...
                    if arch_mov_inv:
                        ...
                        if not df.empty:
                            ...
                            for _, row in df.iterrows():
                                ...
                                is_sender_ger = raw_sender in UNIDADES_GERENCIA_SET
                                is_receiver_ger = raw_receiver in UNIDADES_GERENCIA_SET
                                
                                if not (is_sender_ger or is_receiver_ger) or (is_sender_ger and is_receiver_ger) or 'SERVICIO' in category: continue
                                ...
                                # --- BLOQUE 2: INVENTARIO REGULAR (NO CONSIGNACIÓN) ---
                                    ...
                                    elif is_sender_ger and es_traslado:
                    ...
                    # ====================================================
                    # PROCESAMIENTO 2: ARCHIVO DE VENTAS (SÓLO GERENCIA)
                    # ====================================================
                    if arch_ventas:
