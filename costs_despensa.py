...
def mostrar_modulo_despensa():
    ...
    with tab_carga:
        ...
        with col_m1: mes_proceso = st.selectbox("Mes:", range(1, 13), index=date.today().month - 1, key="m_des")
        with col_m2: anio_proceso = st.number_input("Año:", min_value=2024, value=date.today().year, key="y_des")

        st.markdown("---")
        st.subheader("Archivos Base (Generación Nexus y Auditoría)")
        
        arch_mov_inv = st.file_uploader("1. Reporte de Movimientos (Inventario)", type=["xls", "xlsx"], key="file_inv_des")
        
        col_a1, col_a2 = st.columns(2)
        with col_a1:
            arch_kardex = st.file_uploader("2. Kardex Consolidado (Auditoría)", type=["xls", "xlsx"], key="file_kar_des")
        with col_a2:
            arch_cat = st.file_uploader("3. Maestro Categorías (Opcional)", type=["xls", "xlsx"], key="file_cat_des")

        btn_scan = st.button("🔍 Procesar y Generar Partidas", type="primary", use_container_width=True, key="btn_des")
                    ...
                    if arch_mov_inv:
                        ...
                        if not df.empty:
                            ...
                            for _, row in df.iterrows():
                                ...
                                is_sender_des = raw_sender in UNIDADES_DESPENSA_SET
                                is_receiver_des = raw_receiver in UNIDADES_DESPENSA_SET
                                
                                if not (is_sender_des or is_receiver_des) or (is_sender_des and is_receiver_des) or 'SERVICIO' in category: continue
                                ...
                                if es_ajuste_o_consumo:
                                    if is_receiver_des and not is_sender_des:
                                    ...
                                    elif is_sender_des and not is_receiver_des:
                                        desc = f"RECONOCIMIENTO DE SALIDA POR AJUSTE O CONSUMO DE PRODUCTO DE DESPENSA, MES {mes_proceso} DE {anio_proceso}."
                                ...
                                elif is_sender_des and es_traslado:
