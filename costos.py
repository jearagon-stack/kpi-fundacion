...
def mostrar_modulo_costos():
    ...
    # --- PESTAÑAS: Histórico ahora es la 3 ---
    ...
    with tab1:
        ...
        if not df_ventas.empty:
            ...
            if es_consolidado:
                meses_cons = range(mes_cierre - len(pesos) + 1, mes_cierre + 1)
                filtro &= df_ventas['Fecha_DT'].dt.month.isin(meses_cons)
            else:
        ...
        with st.expander("Carga de Documentos", expanded=True):
            ...
            with ca1: arch_ini = st.file_uploader("1. Inv. Inicial", type=["xlsx"], accept_multiple_files=True)
            with ca2: arch_com = st.file_uploader("2. Compras", type=["xlsx"], accept_multiple_files=True)
            with ca3: arch_fin = st.file_uploader("3. Inv. Final", type=["xlsx"], accept_multiple_files=True)

        if arch_ini and arch_com and arch_fin:
            # --- TODA TU LÓGICA DE PROCESAMIENTO RESTAURADA ---
            ...
            def consolidar(lista):
                return pd.concat([limpiar_nativos(cargar_y_marcar(a)) for a in lista], ignore_index=True)
            ...
            def limpiar_s(s): return s.astype(str).str.strip().str.upper().str.replace(r'\.0$', '', regex=True)
            for d in [df_dic, df_inicial, df_compras, df_final]: d['Codigo'] = limpiar_s(d['Codigo'])
            ...
            consumo_por_cuenta = {}
            costo_op = 0.0
            todas = set(grp_ini.index).union(grp_comp.index).union(grp_fin.index)
            for cta in todas:
                val = grp_ini.get(cta, 0) + grp_comp.get(cta, 0) - grp_fin.get(cta, 0)
                if val > 0: consumo_por_cuenta[cta] = val; costo_op += val
            
            costo_dif_mes = costo_op * porcentaje_subsidio
            costo_real = costo_op - costo_dif_mes + costo_diferido_anterior
            ...
            def gen_excel(filas):
                out = io.BytesIO()
                with pd.ExcelWriter(out, engine='openpyxl') as w: pd.DataFrame(filas).to_excel(w, index=False, header=False)
                return out.getvalue()

            def mostrar_descargas(c_op, c_dif, c_ant, label):
                st.markdown(f"#### 📥 Partidas: {label}")
                conc_v = f"RECONOCIMIENTO COSTO VENTA CAFETERIA, {label} {anio_cierre}."
                f_v = [["410104", "", conc_v, round(c_op, 2), 0.00, ""]]
                for c, m in consumo_por_cuenta.items():
                    m_perc = m * (c_op / costo_op) if costo_op > 0 else 0
                    if m_perc > 0: f_v.append([str(c).replace(".0",""), "", conc_v, 0.00, round(m_perc, 2), ""])
                f_p = [["410104", "", "PROCESO ANT", round(c_ant, 2), 0.00, ""], ["110602", "", "PROCESO ANT", 0.00, round(c_ant, 2), ""]]
                f_d = [["110602", "", "DIFERIMIENTO", round(c_dif, 2), 0.00, ""], ["410104", "", "DIFERIMIENTO", 0.00, round(c_dif, 2), ""]]
                p1, p2, p3 = st.columns(3)
                with p1: st.download_button(f"⬇️ P1 {label}", gen_excel(f_v), f"1_Costo_{label}.xlsx")
                with p2: st.download_button(f"⬇️ P2 {label}", gen_excel(f_p), f"2_Ant_{label}.xlsx")
                with p3: st.download_button(f"⬇️ P3 {label}", gen_excel(f_d), f"3_Dif_{label}.xlsx")

            if not es_consolidado: mostrar_descargas(costo_op, costo_dif_mes, costo_diferido_anterior, mes_txt)
            else:
                for i in range(len(pesos)): mostrar_descargas(costo_op*pesos[i], costo_dif_mes*pesos[i], costo_diferido_anterior*pesos[i], nombres_meses[i])

            def extraer_subunidad(archivo, unidad):
                n = str(archivo).upper()
                sub = ["POLIDEPORTIVO", "ICAS", "JARDINES", "EVENTOS", "CENTRAL", "ABASTECIMIENTO"]
                for s in sub:
                    if s in n: return f"{unidad} {s}"
                return unidad
        ...
        if arch_t:
            try:
                ...
                if df_f_t['Cuenta_Contable'].isna().any():
                    st.error(f"Cod. faltantes: {df_f_t[df_f_t['Cuenta_Contable'].isna()]['Codigo'].unique()}")
                else:
                    grp_t = df_f_t.groupby('Cuenta_Contable')['Monto'].sum()
                    st.success(f"Total: ${grp_t.sum():,.2f}")
                    st.dataframe(grp_t.reset_index()); conc_t = f"TRASLADO {orig_t} A {dest_t}"
                    filas_p = []
                    for c, m in grp_t.items(): filas_p.append([str(c).replace(".0",""), "", conc_t, round(m, 2), 0, ""])
                    for c, m in grp_t.items(): filas_p.append([str(c).replace(".0",""), "", conc_t, 0, round(m, 2), ""])
                    st.download_button("⬇️ Bajar Traslado", gen_excel(filas_p), "Traslado.xlsx")
            except Exception as e: st.error(f"Error: {e}")

    # ==========================================
    # PESTAÑA 3: HISTÓRICO (RESTAURADA AL 100%)
    # ==========================================
    with tab3:
        ...
        if not df_resumen.empty and not df_detalle.empty:
            ...
            if per_sel:
                ...
                # --- MOSTRAR DETALLES SOLO SI SE SOLICITAN ---
                if st.checkbox("📂 Ver Detalle Operativo (Movimientos)"):
                ...
                if st.checkbox("📊 Ver Resumen por Cuentas"):
                    df_det_f = df_detalle[(df_detalle['Mes'].astype(str).str.replace('.0','') == m_f) & (df_detalle['Año'].astype(str).str.replace('.0','') == a_f)]
                ...
                df_det_f = df_detalle[(df_detalle['Mes'].astype(str).str.replace('.0','') == m_f) & (df_detalle['Año'].astype(str).str.replace('.0','') == a_f)]
                df_det_f['Consumo'] = pd.to_numeric(df_det_f['Consumo'], errors='coerce').fillna(0)
                cons_h = df_det_f.groupby('Cuenta')['Consumo'].sum().to_dict()

                conc_v = f"COSTO {m_f}/{a_f}"; f_v_h = [["410104","",conc_v,round(op_h,2),0,""]]
                for c, m in cons_h.items():
                    if m > 0: f_v_h.append([str(c).replace(".0",""), "", conc_v, 0, round(m,2), ""])
                ...
                with ch1: st.download_button("P1 Costo", gen_excel(f_v_h), f"H_P1_{m_f}.xlsx")
                with ch2: st.download_button("P2 Ant", gen_excel(f_p_h), f"H_P2_{m_f}.xlsx")
                with ch3: st.download_button("P3 Dif", gen_excel(f_d_h), f"H_P3_{m_f}.xlsx")