...
from costs_library import mostrar_modulo_libreria as modulo_libreria # <--- Importación de la Librería

def mostrar_modulo_costos():
    # 1. El Menú de Decisión
    unidad = st.radio(
        "🎯 Selecciona la Unidad de Negocio a evaluar:",
        ["☕ Cafetería y Despensa", "🖨️ Talleres Gráficos", "📚 Librería"], # <--- Agregamos Librería al menú
        horizontal=True
    )
    ...
    if unidad == "☕ Cafetería y Despensa":
        ...
        modulo_talleres()
    elif unidad == "📚 Librería": # <--- Desvío hacia la nueva sección
        modulo_libreria()
