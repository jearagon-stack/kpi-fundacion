...
def mostrar_modulo_pedidos():
    ...
    with tab_cajas:
        ...
        try:
                    ...
                    if confirmar:
                        if st.button("🚀 Enviar Pedido a Bodega", type="primary", use_container_width=True):
                            ...
                            if guardar_en_sheets(df_envio):
                                st.session_state['carrito_pedidos'] = pd.DataFrame(columns=["IdProducto", "Descripcion", "MEDIDA", "Cantidad"])
                                # Guardar el mensaje en el estado de sesión para mostrarlo después del rerun
            ...
            # Mostrar la alerta de éxito si existe en la sesión y limpiarla
    ...
    if tab_bodega is not None:
        with tab_bodega:
            ...
            # Botón para forzar la lectura y limpiar la caché
            col_recargar, _ = st.columns([1, 2])
            with col_recargar:
                if st.button("🔄 Recargar Datos de la Nube", use_container_width=True):
                    st.cache_data.clear()
            ...
            try:
                ...
                if df_pendientes.empty:
                    ...
                    st.markdown("Puedes editar cantidades directamente en la tabla o agregar nuevas filas antes de aprobar.")
                    ...
                    with col_dl:
                        ...
                        st.download_button(
                            ...
                            data=datos_excel,
                            file_name=f"Pedido_Aprobado_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            use_container_width=True
                        )
                        
                    with col_del:
                        if st.button("✔️ Marcar todos como Procesados (Limpiar)", type="secondary", use_container_width=True):
                            ws = conectar_hoja("Pedidos_Pendientes")
                            ws.clear()
